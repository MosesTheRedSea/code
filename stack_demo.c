...
int stack_push(int wheels, enum color_t color) {
    ...
    if(capacity >= numCars) {
        ...
        // Check if the result from realloc is NULL /  We've run out of space
        if(tmp == NULL) {
            return FAILURE;
        }
        // If it doesn't fail we just set the new space onthe heap
        
        stack_arr = tmp;
        capacity *= 2;
        // We Update Capacity then add the New Car on the Heap
    }
    ...
    // Don't want to run out of memery we check whether it's NULL
...
int stack_pop(struct car_t *data) {
    // Each individual struct ont he stack once we're no longer holding it ont he array we need to free it

    // TODO
    // UNUSED(data);  <- You don't need this is your code

    // What happens when you calls stack_pop when we have zero cars
    // Write a test case to check if numCars is >= 1
    if (numCars < 1) {
        return FAILURE;
    }

    struct car_t* temp = stack_arr(numCars - 1);
